...
import { Task, Quadrant, AIClassification } from '@/types/task';
...
const mockAIClassify = async (text: string): Promise<AIClassification> => {
    ...
    const urgentKeywords = ['today', 'now', 'asap', 'urgent', 'immediately', 'deadline', 'before', 'pm', 'am', 'hour'];
    const importantKeywords = ['important', 'critical', 'must', 'need', 'required', 'essential', 'pay', 'bill', 'meeting', 'call', 'doctor', 'health'];
    const delegateKeywords = ['someone', 'team', 'help', 'assist', 'assign', 'ask'];
    const lowPriorityKeywords = ['someday', 'maybe', 'later', 'eventually', 'learn', 'try', 'want', 'wish'];
    ...
    const hasLowPriority = lowPriorityKeywords.some(k => lowerText.includes(k));
    
    if (hasUrgent && hasImportant) {
        quadrant = 'do';
        urgency_score = 80 + Math.floor(Math.random() * 20);
        reasoning = 'Classified as DO because it has both a time-sensitive deadline and high importance indicators.';
    } else if (hasImportant && !hasUrgent) {
        quadrant = 'decide';
        urgency_score = 50 + Math.floor(Math.random() * 20);
        reasoning = 'Classified as DECIDE because it\'s important but lacks immediate urgency. Schedule dedicated time for this.';
    } else if (hasUrgent && hasDelegate) {
        quadrant = 'delegate';
        urgency_score = 40 + Math.floor(Math.random() * 20);
        reasoning = 'Classified as DELEGATE because while time-sensitive, this task could be handled by others.';
    } else if (hasUrgent && !hasImportant) {
        quadrant = 'delegate';
        urgency_score = 35 + Math.floor(Math.random() * 20);
        reasoning = 'Classified as DELEGATE because it\'s urgent but not critical to your goals. Consider delegating.';
    } else if (hasLowPriority) {
        quadrant = 'delete';
        urgency_score = 10 + Math.floor(Math.random() * 15);
        reasoning = 'Classified as DELETE because it lacks both urgency and clear importance. Consider if it truly adds value.';
    } else {
        // Default classification
        quadrant = 'decide';
        urgency_score = 30 + Math.floor(Math.random() * 20);
        reasoning = 'Classified as DECIDE by default. Review to determine actual priority level.';
    }
...
export function useTasks(initialTasks: Task[] = []) {
    ...
    const updateTaskQuadrant = useCallback((taskId: number, newQuadrant: Quadrant) => {
    ...
    const getTasksByQuadrant = useCallback((quadrant: Quadrant) => {
        return tasks
            .filter(task => task.quadrant === quadrant && task.status === 'pending')
